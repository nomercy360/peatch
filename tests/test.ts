...
import {Badge, CreateCollaboration, CreateUserCollaboration, UpdateUserRequest} from '../web/gen';
...
describe('Test Admin Routes', () => {
    ...
    const ucRequest: CreateUserCollaboration = {
        user_id: Number('$S{secondUserId}'),
        requester_id: Number('$S{firstUserId}'),
        message: faker.lorem.sentence(),
    }

    it('should create user collaboration request', async () => {
        // first user requests to collaborate with second user
        await spec()
            .post(`${baseUrl}/users/$S{secondCollaborationId}/collaborations`)
            .withHeaders(firstUserAuth)
            .withJson(ucRequest)
            .expectStatus(201);
    })
    ...
    it('should create notification for user', async () => {
        const expectedNotification = {
            message: "user_first_name sends you a message: request_message",
            imgUrl: "http://localhost:3001/api/image?avatar=https%3A%2F%2Fassets.peatch.io%2Fhttps%3A%2F%2Favatars.githubusercontent.com%2Fu%2F59393727&subtitle=District+Directives+Liaison&tags=Dog+Father%2C685155%2Cf149%3BRestaurateur%2C17BEBB%2Ce561%3B&title=Dale+Beahan",
            link: "https://peatch.pages.dev/users/$S{firstUserId}"
        };

        const fs = require('fs');

        const readNotificationFile = async (filename: string) => {
            return new Promise((resolve, reject) => {
                    fs.readFile(filename).then((data: any) => {
         ...
         //const notification = await readNotificationFile('../927635965.json'); // Read the expected file
         //chai.expect(notification).to.equal(expectedNotification);
    });
});