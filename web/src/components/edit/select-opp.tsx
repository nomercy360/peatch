    ...
    <>
      <div class="mt-5 flex h-10 w-full flex-row items-center justify-between rounded-lg bg-secondary px-2.5">
        <input
          ...
          placeholder={t(
            'pages.collaborations.edit.interests.searchPlaceholder',
          )}
          type="text"
          ...
          <For each={filtered()}>
            {(op, index) => (
              <Motion.button
                ...
                class={
                  'flex h-[60px] w-full flex-row items-center justify-start gap-2.5 overflow-hidden rounded-2xl px-2.5'
                }
                initial={{ opacity: 0, x: -20 }}
                animate={{
                  opacity: 1,
                  x: 0,
                  backgroundColor: includes(op.id)
                    ? `#${op.color}`
                    : 'var(--secondary)',
                }}
                transition={{
