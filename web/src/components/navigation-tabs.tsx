...
export default function NavigationTabs(props: any) {
  ...
  return (
    <>
      <div class="fixed bottom-0 z-50 flex h-[100px] w-full items-center justify-between space-x-10 border border-t bg-background px-5 shadow-sm">
        <div class="flex items-center">
          <Link
            href={
              store.user.name &&
              store.user.description &&
              store.user.location?.id
                ? `/users/${store.user?.id}`
                : '/users/edit'
            }
